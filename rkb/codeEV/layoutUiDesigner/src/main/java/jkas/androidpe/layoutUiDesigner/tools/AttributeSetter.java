...
public class AttributeSetter {
    ...
    public void set(View v, Element e) {
        this.element = e;
        this.view = v;
        if (debug != null) debug = LoggerLayoutUI.get(listener.tagNeeded());
    ...
    private void setAttr(View v, Element e) {
        for (Pair<String, String> pair : XmlManager.getAllAttrNValuesFromElement(element)) {
            ...
            /**
             * The two classes below are necessary for applying some attributes. For now, it will
             * stay like this until the next code upgrade.
             */
            ...
            } else if (value.startsWith("@id/")) targetedId = getIdValueAsIntFromListRef(value);
                ...
                switch (name) {
                    ...
                    case "behavior_draggable":
...

                        {
                            ...
                            behavior.setDraggable(Boolean.parseBoolean(value));
                        }
                        continue;
                    ...
                    case "behavior_hideable":
...

                        {
                            ...
                            BottomSheetBehavior behavior =
                                    (BottomSheetBehavior) params.getBehavior();
                            behavior.setHideable(Boolean.parseBoolean(value));
                        }
                        continue;
                    ...
                    case "menu":
                        String path = null;
                        String sVal = value.contains("/") ? value.split("/")[1] : value;
    ...
    private int getId(String idName) {
        ...
        for (Pair<?, ?> pair :
                Utils.convertLinkedHashMapToPairViewElement(
                        listener.onRefViewElementNeeded().getListRef())) {
            element = (Element) pair.second;
            if (element.getAttribute("android:id").equals(idName.replace("@id/", "@+id/"))) {
    ...
    private int getIdValueAsIntFromListRef(String id) {
        if (!id.startsWith("@id/") && !id.equals("parent")) return -1;
