...
public class AttrViewAdapter {
    ...
    private void adapter() {
        ...
        if (listValuesAssist == null || listValuesAssist.length > 1)
    ...
    private void autoCompTextChanged(String txt) {
        ...
        if (valueType != null) {
            if (txt.startsWith("@drawable/")
                ...
                binding.tvAssist.setBackgroundColor(ResourcesValuesFixer.getColor(C, txt));
            }
        }
    }

    private void assist(boolean assist) {
        if (!assist) binding.tvAssist.setVisibility(View.GONE);
    }
    ...
    private void setVFassist() {
        GradientDrawable gradient = new GradientDrawable();
        gradient.setStroke(1, Color.GRAY);
        binding.viewFlipper.setForeground(gradient);
    }
